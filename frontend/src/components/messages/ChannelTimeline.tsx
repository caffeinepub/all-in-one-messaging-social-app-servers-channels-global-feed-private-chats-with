import { useGetMessages, useGetServer, useIsCallerAdmin } from '../../hooks/useQueries';
...
export default function ChannelTimeline({ serverId, channelId }: ChannelTimelineProps) {
  ...
  const { data: messages, isLoading } = useGetMessages(serverId, channelId, pollingEnabled);
  const { data: server } = useGetServer(serverId);
  const { data: isCallerAdmin } = useIsCallerAdmin();

  const currentUserPrincipal = identity?.getPrincipal().toString();
  const isServerOwner = server && currentUserPrincipal === server.ownerPrincipal.toString();
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 pb-4">
            {messages.map((message, index) => {
              ...
              const canDelete = isAuthor || isServerOwner || isCallerAdmin;
              return (
                <MessageItem 
