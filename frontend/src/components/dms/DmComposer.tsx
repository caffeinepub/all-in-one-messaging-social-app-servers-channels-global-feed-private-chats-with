...
import { useSendDM } from '../../hooks/useQueries';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('profile required')) {
        toast.error('You need to set up your profile before sending messages');
      } else if (errorMessage.includes('not found')) {
        toast.error('Recipient not found');
      } else if (errorMessage.includes('Unauthorized')) {
        toast.error('You are not authorized to send this message');
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2">
      ...
      <div className="flex items-end gap-2 relative">
        ...
        <Button type="submit" size="icon" disabled={!hasContent || sendDm.isPending}>
          {sendDm.isPending ? (
