import { useGetDMMessages } from '../../hooks/useQueries';
...
export default function DmTimeline({ otherUserPrincipal }: DmTimelineProps) {
  ...
  const { data: messages, isLoading } = useGetDMMessages(otherUserPrincipal, pollingEnabled);
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 pb-4">
            {messages.map((message, index) => (
              <MessageItem key={`${message.id}-${index}`} message={message} canDelete={false} />
            ))}
          </div>
        ) : (
          <EmptyState
